...
name: autonomic
version: 1.0.0
description: "Master autonomous product orchestrator — takes an app or feature idea from prompt to shipped PR by chaining product discovery, formal specs, spec-derived test contracts, architecture, tickets, /harness TDD or /team-cheap swarms, and verification gates automatically."
scope: global
user-invocable: true
dependencies: [product-function, find-skills, to-spec, unslop, spec-to-tests, information-architecture-review, ooux, to-tickets, harness, team-cheap, code-review]
allowed-tools:
  - define_subagent
  - invoke_subagent
  - send_message
...
flowchart TD
    ...
    B --> C[Step 2: /to-spec + Gate 1: /unslop]
    C --> D[Step 3: /spec-to-tests Test Contract]
    D --> E[Step 4: /ia & /ooux Architecture]
    E --> F[Step 5: /to-tickets Bound to Scenarios]
    F --> G{Module Topology Decision}
    G -- Single Module / Sequential --> H[Step 6a: /harness TDD Worktree]
    G -- Multi-Module / Swarm --> I[Step 6b: /team-cheap Subagent Swarm]
    H --> J[Step 7: ./init.sh + .gga Review]
    I --> J
    J --> K[Step 8: Gate 2: /unslop PR + Open GitHub PR]
```
...
### Step 2: Formal Specifications & Gate 1 (`/to-spec` + `/unslop`)
...
### Step 3: Spec-Derived Behavioral Test Contract (`/spec-to-tests`)
...
### Step 4: Information Architecture & Domain Modeling (`/ia` & `/ooux`)
...
### Step 5: Atomic Ticket Decomposition (`/to-tickets`)
...
### Step 6: Autonomous Execution Routing (`/harness` vs `/team-cheap`)
...
### Step 8: Delivery & Gate 2 (`/unslop` PR)
